...
- Title row: `h1.text-display.font-bold.text-text.print:text-2xl`. Actions: `.stepper` (`−`, input `#scale`, `+`; label `for="scale"` kept visible as `text-sm font-medium text-muted mr-2` as on main), Edit `.btn`, Add to shopping list `.btn`, Cook `.btn.btn-primary`, static-mode download `.btn`. Labels keep main's `hidden lg:inline` spans: with four labelled 40px buttons plus the stepper, the title row wraps between 768 and 1023px, which main avoided by showing icons only there. Each button carries a `title` for the icon-only state.
