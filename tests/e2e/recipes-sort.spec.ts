...
/**
 * Coverage for the recipes-index sorter. It previously had none, which is how
 * it was removed and restored without a single test changing colour.
 *
 * Assertions read data-name rather than the card's heading text so that these
 * tests do not re-create the markup coupling the sorter itself was fixed to
 * avoid.
 */
...
test.describe('Recipes index sorting', () => {
  ...
  test.beforeEach(async ({ page }) => {
    ...
    // Each test starts from the default ordering, not whatever a previous
    // test persisted.
